...
def test_build_training_dataset_emits_event_tail_features() -> None:
    start = dt.date(2024, 1, 1)
    rows = []
    for day in range(30):
        close = 100.0
    ...
    dataset = build_training_dataset(pd.DataFrame(rows), min_history_days=20)
    last_row = dataset.loc[dataset["event_volume_spike_20d"].eq(1.0)].iloc[0]

    assert last_row["feature_version"] == "feature_eod_tabular_v4"
    assert last_row["event_volume_spike_20d"] == 1.0
    assert last_row["event_financial_volume_spike_z20"] == 1.0
    assert last_row["event_abs_return_5d_ge_10"] == 1.0
    assert last_row["event_tail_risk_score"] > 0.0
...
def test_align_config_to_dataset_uses_snapshot_versions() -> None:
    ...
    config = BaselineMlpConfig(feature_version="feature_eod_tabular_v4")
