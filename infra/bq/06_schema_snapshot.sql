...
--
-- Uso:
-- 1) Edite project_id, dataset_id, metadata_region e (opcional) tables_filter.
...
DECLARE metadata_region STRING DEFAULT 'us';
...
-- Verifica se o dataset existe na região informada para evitar erro "Dataset ... was not found in location ...".
EXECUTE IMMEDIATE FORMAT(
  ...
  ''',
  project_id,
  LOWER(metadata_region)
)
INTO dataset_exists
USING dataset_id AS dataset_id;

IF dataset_exists THEN
  -- Resultado 1: metadados por tabela (tipo, criação e DDL)
  EXECUTE IMMEDIATE FORMAT(
    '''
    SELECT
...
ELSE
  -- Resultado único de diagnóstico quando o dataset não existe na região.
  SELECT
    FORMAT(
      'Dataset %s.%s não encontrado na região %s. Ajuste metadata_region ou crie o dataset antes de executar o snapshot.',
      project_id,
      dataset_id,
      UPPER(metadata_region)
    ) AS warning_message,
    project_id,
    dataset_id,
    UPPER(metadata_region) AS metadata_region,
    tables_filter AS requested_tables;
END IF;
