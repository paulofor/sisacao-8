...
Próximo passo operacional: não criar Scheduler novo nem ampliar uso automaticamente. Monitorar o champion aprovado em modo operacional controlado: validar se as APIs/telas consomem apenas o modelo `approved`, acompanhar sinais gerados, conferir se os tickers bloqueados (`ONCO3`, `VVEO3`, `AMBP3`) continuam fora da política e auditar diariamente `neural_daily_returns`, `neural_gate_decisions` e incidentes. Qualquer promoção para execução com capital real ou alteração de Scheduler deve exigir nova autorização explícita.
