...
## 2026-07-09 16:05 UTC — Próximo passo: snapshot v4 com features de evento extremo
- O próximo passo estrutural foi implementado no código: `feature_eod_tabular_v4` adiciona flags/score de evento extremo (`event_volume_spike_20d`, `event_financial_volume_spike_z20`, `event_abs_return_5d_ge_10`, `event_tail_risk_score`) para expor casos como `RCSL3` 2026-06-24 ao modelo.
- Antes de qualquer novo treino recorrente, aplicar o DDL v4 no BigQuery e redeployar `neural_training_dataset`/`neural_training`.
- Depois gerar um novo snapshot v4 e validar no BigQuery que as quatro colunas existem e que eventos extremos neutros têm flags/score altos.
- Primeira rodada após o snapshot v4 deve ser uma shadow pequena e interpretável tabular; só voltar para TCN/GRU/LSTM se o v4 reduzir caudas sem depender de blocklist manual.
- Manter sem promoção e sem Scheduler até um Gate MUEN `passed` com estabilidade multi-seed.

## 2026-07-09 16:25 UTC — DDL v4 executado; seguir para deploy e snapshot
- O DDL v4 foi informado como executado pelo usuário.
- A validação automática via MCP/BigQuery ficou pendente porque `http://mcpserversisacao.shop/mcp` retornou `503 Service Unavailable`; repetir a checagem em `INFORMATION_SCHEMA.COLUMNS` quando o MCP voltar.
- Próximo passo agora: redeployar `neural_training_dataset` e `neural_training`, gerar um snapshot `feature_eod_tabular_v4`, validar as colunas/event scores e só então rodar uma shadow tabular pequena.

## 2026-07-09 16:35 UTC — Retry MCP ainda indisponível
- Nova tentativa de validar o DDL v4 pelo MCP HTTP/JSON-RPC retornou `503 Service Unavailable` já no `initialize`.
- Não trocar para HTTPS. Repetir a validação de schema quando o MCP voltar.
- Enquanto isso, o próximo passo operacional continua sendo redeployar `neural_training_dataset` e `neural_training`, gerar snapshot v4 e validar as colunas antes de qualquer shadow.

## 2026-07-09 16:45 UTC — Schema v4 validado; próximo passo é deploy/snapshot
- A validação BigQuery confirmou que `neural_eod_training_dataset` já possui as quatro colunas v4: `event_abs_return_5d_ge_10`, `event_financial_volume_spike_z20`, `event_tail_risk_score` e `event_volume_spike_20d`.
- Próximo passo agora: redeployar `neural_training_dataset` e `neural_training` com o código v4, gerar um snapshot novo `feature_eod_tabular_v4` e validar os valores das novas features em eventos extremos.
- Não executar nova shadow ainda; primeiro confirmar que o snapshot v4 foi gerado com dados reais nas novas colunas.

## 2026-07-09 17:05 UTC — Deploy v4 ainda pendente; snapshot `event_v4` atual é v3
- A tentativa de gerar o snapshot `neural_eod_training_dataset_2026-06-27_event_v4` mostrou que a Cloud Function publicada ainda retorna `feature_version=feature_eod_tabular_v3`.
- Não usar esse snapshot para treino/shadow: ele tem nome v4, mas manifesto v3. A tentativa de deletar pelo MCP não funcionou porque a ferramenta BigQuery disponível aceita apenas queries read-only.
- Próximo passo obrigatório: redeployar `neural_training_dataset` e `neural_training` com o código v4. Depois recriar o snapshot com `replace_snapshot=true` e confirmar que o manifesto retorna `feature_eod_tabular_v4` antes de qualquer shadow.

## 2026-07-10 00:15 UTC — MCP não consegue publicar funções; deploy externo obrigatório
- O MCP Server foi consultado por HTTP JSON-RPC. Ele não expõe ferramenta de deploy de Cloud Functions; `gcloud_research` é somente-leitura.
- A leitura das funções publicadas mostrou `DEPLOY_GITHUB_SHA=7b93c9308530a08bedefe04ae6702d042c9819cb`, diferente do HEAD local `052d52f34398491e845fda54fd70b1b0bca19a53`.
- Portanto, `neural_training_dataset` e `neural_training` ainda precisam ser redeployadas pelo workflow/Actions ou por `gcloud` externo antes de gerar snapshot v4 válido.
- Depois do deploy, recriar `neural_eod_training_dataset_2026-06-27_event_v4` com `replace_snapshot=true` e aceitar somente se o manifesto vier com `feature_version=feature_eod_tabular_v4`.
