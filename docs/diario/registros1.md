...
## 2026-07-08 18:10 UTC — TCN ticker_v3 pós-deploy com daily returns persistidos
- Contexto: após o deploy do código que materializa `daily_returns` e retorna `daily_return_count`, executei o próximo passo operacional definido para a Fase 4: repetir a TCN `p50/m08/t20/d15/l20` com sufixo novo `_ticker_v3` e três seeds.
- Dry-run: o `neural_evolution_orchestrator` retornou HTTP 200 com `candidate_count=3`, `architecture_types=["tcn_sequence"]`, snapshot `neural_eod_training_dataset_2026-06-27_313c9df2` e candidatos esperados para a família `neural_eod_phase4_tcn_sequence_p50_m08_t20_d15_l20_ticker_v3`.
- Rodada real: o endpoint retornou HTTP 200 em aproximadamente 143 segundos com `trained_count=3`, `evaluated_count=3`, `failed_count=0`, `fold_metric_count=12`, `family_evaluation_count=1`, `gate_decision_count=1` e `daily_return_count=9900`.
- Validação BigQuery via MCP: a decisão MUEN consolidada da família foi `rejected`, com `seeds=3`, `total_trades=68`, `positive_folds=2`, `positive_fold_ratio=0.16666666666666666`, `median_delta=-0.003538582548495635`, `worst_delta=-0.07000000000000006`, `max_drawdown=0.1956430000000005` e `stable_across_seeds=false`.
- Critérios de reprovação: `folds_positivos_insuficientes`, `nao_supera_champion_mediana`, `fold_catastrofico` e `seeds_instaveis`. Conclusão: a persistência está correta, mas a família não tem qualidade financeira suficiente para promoção ou agendamento recorrente.
- Diagnóstico por ticker/data/fold: a consulta de piores linhas em `neural_daily_returns` agora retornou tickers, confirmando a rastreabilidade. As perdas extremas se concentraram em casos nos quais o champion estava neutro (`champion_net_return=0`) e a TCN assumiu exposição com perda próxima de 7%, principalmente em `ONCO3` nos dias 2026-06-08, 2026-06-09, 2026-06-10 e 2026-06-19, além de `BRKM5` e `CSAN3` em janelas de validation/test.
- Decisão operacional: não promover, não criar Scheduler dedicado e não continuar rodando variações de arquitetura/stop até investigar a causa das entradas de cauda. O próximo desenvolvimento deve criar diagnóstico/filtro de regime, liquidez, evento ou neutralização por ticker/data antes de novo treino Fase 4.
- Comandos usados: Python `urllib.request` com `/tmp/phase4_tcn_d15_ticker_v3_payload.json`; MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp` com `initialize`, `tools/list` e `tools/call bigquery_query`; consultas em `neural_gate_decisions` e `neural_daily_returns`.

## 2026-07-08 18:35 UTC — Implementação de guarda de cauda por ticker
- Implementei uma primeira versão objetiva do diagnóstico/filtro de cauda recomendado após a TCN ticker_v3: o parâmetro `blocked_tickers` passa a ser aceito no treinamento e na geração de candidatos.
- No treinamento, `apply_ticker_blocklist` neutraliza decisões direcionais (`up`/`down`) dos tickers configurados, mantendo as demais decisões intactas. A aplicação ocorre depois do orçamento `max_trades_per_fold` e antes do `max_fold_drawdown_stop`, para que tickers bloqueados não consumam drawdown intrafold.
- No endpoint `neural_training`, o payload aceita `blocked_tickers` como lista ou string separada por vírgulas, normalizando para maiúsculas. O valor entra no manifest/registry via `hyperparameters_json`.
- No gerador de evolução, `blocked_tickers` é propagado de `family_space` para hiperparâmetros e `training_request`; também entra no hash de família e no sufixo de versão como `btN_<hash>`, evitando colisão com a TCN ticker_v3 original.
- Cobertura adicionada: teste unitário para neutralização por ticker, teste de economia MUEN aplicando a guarda antes das métricas, teste de propagação/versionamento no gerador Phase 4 e teste do endpoint de treinamento persistindo `blocked_tickers`.
- Próximo passo pós-deploy: rodar uma TCN shadow com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]`, três seeds, `p50/m08/t20/d15/l20`, sem promoção automática, e comparar contra a TCN ticker_v3.
- Comandos usados: `rg -n "max_fold_drawdown_stop|prepare_sequence_training_arrays|min_directional|daily_return_rows|apply_fold_drawdown_stop|BaselineMlpConfig" sisacao8 functions/neural_training functions/neural_evolution_orchestrator tests`; scripts Python de edição; `python -m black sisacao8/neural_training.py functions/neural_training/sisacao8/neural_training.py functions/neural_training/main.py sisacao8/neural_evolution.py functions/neural_evolution_orchestrator/sisacao8/neural_evolution.py tests/test_neural_training.py tests/test_neural_evolution.py tests/test_neural_training_function.py`.

## 2026-07-08 20:05 UTC — Execução TCN com guarda `blocked_tickers`
- Executei o dry-run da família TCN Fase 4 `p50/m08/t20/d15/l20` com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]`; a função publicada já reconheceu a guarda, gerando três candidatos com sufixo `bt3_407e4c`.
- Executei a rodada real pelo `neural_evolution_orchestrator`; resposta HTTP 200 em aproximadamente 135 segundos, com `trained_count=3`, `evaluated_count=3`, `failed_count=0`, `daily_return_count=9900`, `fold_metric_count=12`, `family_evaluation_count=1` e `gate_decision_count=1`.
- Validação via MCP/BigQuery: a família `neural_eod_phase4_tcn_sequence_p50_m08_t20_d15_l20_bt3_onco3_brkm5_csan3_v1` foi rejeitada por `fold_catastrofico`, mas melhorou em estabilidade e mediana: `seeds=3`, `total_trades=52`, `positive_folds=6`, `positive_fold_ratio=0.5`, `median_delta=0.007912287316774284`, `worst_delta=-0.03500000000000004`, `max_drawdown=0.19564300000000023`, `stable_across_seeds=true`.
- Comparação operacional contra a TCN ticker_v3: a blocklist removeu `seeds_instaveis`, melhorou a mediana de negativa para positiva e reduziu o pior delta de aproximadamente `-0.07` para `-0.035`, mas ainda não passa no Gate MUEN porque restou fold catastrófico.
- Diagnóstico das novas caudas: `neural_daily_returns` mostrou perdas em `ARML3` nos dias 2026-06-24 e 2026-06-25 e em `RCSL3` nos dias 2026-06-10 e 2026-06-18, em operações nas quais o champion ficou neutro e a TCN assumiu exposição.
- Decisão: não promover, não criar Scheduler dedicado e não continuar expandindo blocklist manual ticker a ticker. O próximo passo deve ser uma regra geral e explicável para esse padrão de cauda, como filtro de regime/liquidez/evento ou concordância/compatibilidade com o champion quando a rede quiser operar contra neutralidade.
- Comandos usados: Python `urllib.request` para dry-run e execução real; MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp`; consultas BigQuery em `neural_gate_decisions`, `INFORMATION_SCHEMA.COLUMNS` e `neural_daily_returns`.

## 2026-07-08 20:40 UTC — Implementação do filtro `require_champion_activity`
- Implementei o próximo desenvolvimento indicado pela rodada TCN com blocklist: em vez de continuar adicionando tickers manualmente, criei uma regra geral para entradas contra champion neutro.
- O novo parâmetro `require_champion_activity` entra em `BaselineMlpConfig` e no endpoint `neural_training`. Quando ativo, `apply_champion_activity_filter` neutraliza labels direcionais em linhas com `champion_net_return` igual a zero.
- A ordem de aplicação ficou: política conservadora de probabilidade/margem, orçamento `max_trades_per_fold`, `blocked_tickers`, `require_champion_activity`, `max_fold_drawdown_stop` e então economia MUEN. Assim a regra atua antes de medir drawdown e não altera o Gate MUEN.
- O gerador de evolução propaga `require_champion_activity` de `family_space` para hiperparâmetros e `training_request`, inclui o parâmetro no hash de família e adiciona sufixo `ca` ao `model_version`.
- Testes adicionados/ajustados cobrem neutralização por champion neutro, aplicação dentro da economia MUEN, propagação em Phase 4 e persistência no endpoint de treinamento.
- Próximo passo pós-deploy: rodar TCN shadow com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]` e `require_champion_activity=true`; se `fold_catastrofico` persistir ou trades caírem demais, encerrar essa linha de Fase 4 e revisar labels/features/regime.
- Comandos usados: `rg -n "champion|baseline|sinal|signal|predicted_label|label_class" sisacao8 functions tests`; scripts Python de edição; `python -m black sisacao8/neural_training.py functions/neural_training/sisacao8/neural_training.py functions/neural_training/main.py sisacao8/neural_evolution.py functions/neural_evolution_orchestrator/sisacao8/neural_evolution.py tests/test_neural_training.py tests/test_neural_evolution.py tests/test_neural_training_function.py`.

## 2026-07-09 02:30 UTC — Falha controlada do filtro `require_champion_activity` e correção de compatibilidade
- Executei o próximo passo pós-deploy: dry-run e rodada real da TCN Fase 4 com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]` e `require_champion_activity=true`.
- O dry-run retornou HTTP 200 e confirmou que a função publicada reconhece o parâmetro, gerando modelos com sufixo `bt3_407e4c_ca`.
- A execução real retornou HTTP 500 no orquestrador porque os três chamados a `neural_training` falharam. A resposta operacional veio com `trained_count=0`, `evaluated_count=0`, `failed_count=3`, `daily_return_count=0`, `fold_metric_count=0` e `gate_decision_count=0`.
- Diagnóstico confirmado via logs MCP: `neural_training` levantou `ValueError: champion_net_return column is required when require_champion_activity is enabled` dentro de `apply_champion_activity_filter`.
- Validação BigQuery via `INFORMATION_SCHEMA.COLUMNS` confirmou que `neural_eod_training_dataset` possui `buy_net_return`, `sell_net_return`, `net_return` e `trade_side`, mas não possui `champion_net_return` nem sinal real do champion. Portanto, a leitura anterior de champion neutro vinha do fallback zero da avaliação MUEN, não de uma coluna materializada.
- Correção aplicada: `align_config_to_dataset` agora desativa `require_champion_activity` quando o snapshot não contém `champion_net_return`, evitando 500 e registrando que esse filtro não pode ser validado no dataset atual.
- Próximo passo: publicar a correção e, antes de nova tentativa `ca`, evoluir a materialização do dataset para incluir atividade/retorno do champion de forma point-in-time. Sem isso, a rodada `ca` seria apenas no-op e não deve ser usada como evidência de aprovação.
- Comandos usados: Python `urllib.request` para dry-run/rodada real; MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp` com `cloud_run_function_logs` e `bigquery_query`; `rg -n "buy_net_return|sell_net_return|champion_net_return|return" sisacao8/neural_dataset.py functions/neural_training/sisacao8/neural_dataset.py infra/bq`; edição de `sisacao8/neural_training.py`, `functions/neural_training/sisacao8/neural_training.py` e `tests/test_neural_training.py`.

## 2026-07-09 03:05 UTC — Materialização de `champion_net_return` no dataset neural
- Executei o próximo desenvolvimento exigido pelo erro do filtro `require_champion_activity`: adicionar a atividade/retorno do champion ao dataset point-in-time, em vez de depender do fallback zero da avaliação MUEN.
- Validei via MCP/BigQuery que `quant_backtest_trades` possui trades de estratégias baseline, incluindo `baseline_daily_momentum_v1`, com `ticker`, `reference_date`, `side`, `strategy_id`, `strategy_version` e `net_pnl_pct`.
- Atualizei `neural_training_dataset` para carregar os trades do champion/baseline configurado por `NEURAL_CHAMPION_STRATEGY_ID` ou payload `champion_strategy_id`, com filtros opcionais `champion_strategy_version` e `champion_config_version`.
- O dataset final agora faz left join por `ticker` e `reference_date` e materializa `champion_strategy_id`, `champion_strategy_version`, `champion_signal_side`, `champion_net_return` e `champion_trade_active`; linhas sem trade ficam com `champion_net_return=0.0` e `champion_trade_active=false`.
- Atualizei `TRAINING_DATASET_COLUMNS` e o DDL `infra/bq/17_neural_eod_training_dataset.sql` com `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` para as novas colunas.
- Atualizei os testes da Cloud Function de dataset para simular `quant_backtest_trades` e garantir que as colunas do champion são carregadas e que ao menos uma linha tem `champion_trade_active=true`.
- Próximo passo operacional: aplicar o DDL no BigQuery, redeployar `neural_training_dataset`, gerar novo snapshot e repetir a TCN `blocked_tickers+require_champion_activity` somente sobre snapshot com `champion_net_return` real.
- Comandos usados: `rg -n "backtest|sinais_eod|champion|strategy|signal|sinal|neural_eod_training_dataset|TRAINING_DATASET" functions/neural_training_dataset functions sisacao8 infra tests`; MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp` com consultas BigQuery em `quant_backtest_trades`; `python -m pytest tests/test_neural_training_dataset_function.py tests/test_neural_training.py -q`.

## 2026-07-09 03:35 UTC — Checagem do DDL champion e snapshot sem dados preenchidos
- O usuário aplicou o DDL no BigQuery. Validei via MCP/BigQuery que `neural_eod_training_dataset` agora possui `champion_strategy_id`, `champion_strategy_version`, `champion_signal_side`, `champion_net_return` e `champion_trade_active`.
- Executei a Cloud Function `neural_training_dataset` com payload `dataset_snapshot=neural_eod_training_dataset_2026-06-27_champion_v1`, `end_date=2026-06-27`, `replace_snapshot=true`, `champion_strategy_id=baseline_daily_momentum_v1`.
- A função retornou HTTP 200 em aproximadamente 27 segundos e criou `9044` linhas (`train=5894`, `validation=750`, `test=900`, `embargo=1500`).
- A consulta de validação do snapshot mostrou `champion_active_rows=0`, `champion_return_rows=0` e `champion_strategy_ids=0`. Conclusão: a tabela está com o schema certo, mas a função publicada ainda não contém a implementação do join em `quant_backtest_trades`; ela carregou as novas colunas como nulas.
- Decisão operacional: não rodar ainda TCN `require_champion_activity=true`, porque o snapshot novo não tem dados reais do champion. O próximo passo é redeployar `neural_training_dataset` com o código mais recente, recriar o mesmo snapshot com `replace_snapshot=true` e validar `champion_active_rows > 0`.
- Comandos usados: MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp`; consulta `INFORMATION_SCHEMA.COLUMNS`; Python `urllib.request` para `https://us-east1-ingestaokraken.cloudfunctions.net/neural_training_dataset`; consulta agregada em `neural_eod_training_dataset` para `champion_active_rows`.

## 2026-07-09 08:10 UTC — Snapshot champion pós-deploy e TCN `bt3+ca`
- O usuário confirmou o deploy do `neural_training_dataset` atualizado; reexecutei a geração do snapshot `neural_eod_training_dataset_2026-06-27_champion_v1` com `replace_snapshot=true` e `champion_strategy_id=baseline_daily_momentum_v1`.
- A Cloud Function retornou HTTP 200 em aproximadamente 36 segundos, recriando `9044` linhas com splits `train=5894`, `validation=750`, `test=900` e `embargo=1500`.
- Validação BigQuery via MCP confirmou que o snapshot agora contém dados reais do champion: `champion_active_rows=718`, `champion_return_rows=9044`, `champion_strategy_ids=1`, datas de 2026-03-30 a 2026-06-25. Portanto a hipótese de falta de deploy foi resolvida.
- Executei dry-run da TCN Fase 4 com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]` e `require_champion_activity=true`; a resposta HTTP 200 confirmou `candidate_count=3`, snapshot correto e sufixos `bt3_407e4c_ca` nos três candidatos.
- Executei a rodada real da mesma família. O orquestrador retornou HTTP 200 em aproximadamente 154 segundos, com `trained_count=3`, `evaluated_count=3`, `failed_count=0`, `daily_return_count=9900`, `fold_metric_count=12`, `family_evaluation_count=1` e `gate_decision_count=1`.
- Resultado MUEN consolidado: a família `neural_eod_phase4_tcn_sequence_p50_m08_t20_d15_l20_bt3_ca_v2` foi `rejected`, com `seeds=3`, `total_trades=0`, `positive_folds=0`, `positive_fold_ratio=0.0`, `median_delta=0.0`, `worst_delta=0.0`, `max_drawdown=0.0`, `stable_across_seeds=false` e falhas `trades_insuficientes`, `folds_positivos_insuficientes`, `nao_supera_champion_mediana`, `seeds_instaveis`.
- Leitura operacional: o filtro `require_champion_activity` funcionou como guarda conservadora, removendo drawdown e caudas, mas também neutralizou todas as operações da TCN nesta configuração. Isso não é candidato a promoção nem a Scheduler; a linha Fase 4 TCN `bt3+ca` deve ser encerrada como excessivamente restritiva para o snapshot atual.
- Próximo passo: parar variações incrementais da TCN recorrente e voltar para hipótese estrutural de dados/target. Priorizar análise de features/labels/regimes usando as colunas `champion_*` já materializadas, buscando uma regra que preserve trades suficientes sem operar contra eventos de cauda. Só reabrir treino Fase 4 quando houver nova hipótese de feature/label/regime, não apenas mais blocklist/stop.
- Comandos usados: Python `urllib.request` para `neural_training_dataset` e `neural_evolution_orchestrator`; MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp` com `bigquery_query` para validar o snapshot, `neural_gate_decisions` e `neural_daily_returns`.

## 2026-07-09 08:45 UTC — Próximo passo preparado: diagnóstico champion/regime
- Tentei executar a análise BigQuery via MCP HTTP/JSON-RPC, mas o endpoint `http://mcpserversisacao.shop/mcp` retornou `503 Service Unavailable` em múltiplas tentativas com backoff; mantive a regra operacional de não trocar para HTTPS.
- Como continuação do próximo passo, criei `docs/implementacao/diagnostico-neural-champion-regime.sql` com um pacote reprodutível de consultas para validar o snapshot champion, medir sobreposição da TCN `bt3` com `champion_trade_active`, comparar regimes/features entre champion ativo e neutro, listar piores ticker/data/fold e confirmar a rejeição `bt3+ca` por trades zerados.
- A decisão operacional segue sem promoção e sem Scheduler: executar esse SQL no BigQuery/MCP quando o MCP voltar, interpretar se há regime/feature que preserve cobertura sem operar contra caudas e só então propor nova hipótese de treino.
- Comandos usados: `python /tmp/mcp_query.py` com MCP HTTP JSON-RPC e retries; criação de `docs/implementacao/diagnostico-neural-champion-regime.sql`; `rg` para confirmar nomes de colunas disponíveis no dataset.

## 2026-07-09 09:05 UTC — Nova tentativa MCP para diagnóstico champion/regime
- Reexecutei a tentativa solicitada do diagnóstico champion/regime usando MCP HTTP/JSON-RPC em `http://mcpserversisacao.shop/mcp`, com script de retry e backoff para `initialize` e `tools/call bigquery_query`.
- O endpoint continuou retornando `503 Service Unavailable` durante a inicialização da sessão, antes de conseguir executar a consulta de sanidade do snapshot champion.
- Mantive a regra operacional obrigatória de usar HTTP e não trocar para HTTPS. Nenhuma nova decisão de treino/promoção foi tomada sem a consulta BigQuery.
- Próximo passo permanece: repetir a execução do SQL `docs/implementacao/diagnostico-neural-champion-regime.sql` quando o MCP voltar; até lá, sem promoção, sem Scheduler e sem novas variações TCN.
- Comandos usados: `python /tmp/mcp_query.py` com consulta de sanidade em `neural_eod_training_dataset` e retries HTTP JSON-RPC.

## 2026-07-09 09:25 UTC — Diagnóstico champion/regime executado com sucesso
- Repeti a execução via MCP HTTP/JSON-RPC e o serviço voltou a responder. A consulta de sanidade confirmou o snapshot `neural_eod_training_dataset_2026-06-27_champion_v1` com `9044` linhas, `champion_active_rows=718`, `champion_return_rows=9044`, `champion_strategy_ids=1`, período de 2026-03-30 a 2026-06-25.
- A sobreposição entre a TCN `bt3` e o champion real explicou por que `require_champion_activity=true` zerou a rodada `bt3+ca`: os `52` trades da TCN `bt3` ocorreram todos com `champion_trade_active=false`; `overlap_champion_active_rows=0`, `trades_when_champion_active=0`, `trades_when_champion_neutral=52`.
- A TCN `bt3` ainda tinha média positiva (`avg_delta=0.00750746917739292`), mas o pior delta permaneceu `-0.07000000000000013`, mostrando que o problema não é apenas cauda isolada: a família encontrou edge fora do domínio operacional do champion e, ao exigir champion ativo, a cobertura vai a zero.
- O regime médio das linhas operadas pela TCN `bt3` foi de momentum/liquidez fracos: `avg_return_5d=-0.01606386505543051`, `avg_log_return_5d=-0.016396402272924552`, `avg_financial_volume_z20=-0.16317770062582426`, `avg_volume_ratio_20d=0.9067599264055777`, `avg_volatility_20d=0.0189825908085329`.
- Em contraste, no conjunto validation/test, as poucas linhas em que o champion estava ativo apresentaram regimes de volume/momentum bem mais fortes: por exemplo `label_class=down` com `rows_count=11`, `avg_financial_volume_z20=1.0054798281207902`, `avg_volume_ratio_20d=1.6005469561459673`, `avg_return_5d=0.08242689646944032`; `label_class=up` com `rows_count=10`, `avg_financial_volume_z20=0.9338776327147861`, `avg_volume_ratio_20d=1.4208470538669165`, `avg_return_5d=0.07633105688100701`.
- As piores linhas confirmaram cauda em champion neutro: `ARML3` em 2026-06-24/2026-06-25 e `RCSL3` em 2026-06-10/2026-06-18, todas com `champion_trade_active=false`, exposição `1.0` e `delta_net_return` próximo de `-0.07`.
- A decisão da família `bt3+ca` foi confirmada como `rejected`, com `total_trades=0`, `positive_folds=0`, `median_delta=0.0`, `worst_delta=0.0`, `max_drawdown=0.0`, `stable_across_seeds=false` e falhas `trades_insuficientes`, `folds_positivos_insuficientes`, `nao_supera_champion_mediana`, `seeds_instaveis`.
- Conclusão operacional: não repetir TCN `bt3+ca` nem promover. O próximo desenvolvimento deve substituir o filtro binário `require_champion_activity` por uma hipótese de regime/liquidez/momentum mais suave e testável, por exemplo um guard opcional que só permita trades quando `financial_volume_z20`/`volume_ratio_20d` e `return_5d` indicarem regime semelhante ao domínio em que o champion opera, antes de nova rodada shadow.
- Comandos usados: `python /tmp/mcp_query.py` via MCP HTTP JSON-RPC em `http://mcpserversisacao.shop/mcp`; consultas do pacote `docs/implementacao/diagnostico-neural-champion-regime.sql` em `neural_eod_training_dataset`, `neural_daily_returns` e `neural_gate_decisions`.

## 2026-07-09 10:05 UTC — Implementação da guarda regime/liquidez/momentum
- Executei o próximo desenvolvimento indicado pelo diagnóstico champion/regime: criei uma guarda configurável e mais suave que `require_champion_activity`, baseada em colunas já existentes do dataset (`return_5d`, `financial_volume_z20`, `volume_ratio_20d`).
- A nova função `apply_regime_liquidity_filter` neutraliza decisões direcionais quando uma linha não atende aos thresholds opcionais `min_regime_return_5d`, `min_regime_financial_volume_z20` e/ou `min_regime_volume_ratio_20d`.
- A guarda é aplicada depois de `blocked_tickers` e `require_champion_activity` e antes de `max_fold_drawdown_stop`, preservando o Gate MUEN e mantendo o controle audível no payload.
- Propaguei os três novos hiperparâmetros em `BaselineMlpConfig`, no endpoint `neural_training`, no `training_request`, no hash de família e no sufixo de versão como `rg_<hash>`, evitando colisão entre políticas.
- Atualizei testes para cobrir neutralização de linhas fora de regime, aplicação antes das métricas MUEN, propagação via Phase 4 e persistência dos novos hiperparâmetros no endpoint.
- Próximo passo pós-deploy: rodar uma TCN shadow pequena com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]`, sem `require_champion_activity`, e com thresholds iniciais derivados do diagnóstico (`min_regime_return_5d=0.0`, `min_regime_financial_volume_z20=1.0`, `min_regime_volume_ratio_20d=1.4`). Só comparar; não promover sem Gate MUEN `passed`.
- Comandos usados: `rg` para localizar guards/propagação; edição de `sisacao8/neural_training.py`, `functions/neural_training/sisacao8/neural_training.py`, `functions/neural_training/main.py`, `sisacao8/neural_evolution.py`, `functions/neural_evolution_orchestrator/sisacao8/neural_evolution.py` e testes; `python -m pytest tests/test_neural_training.py tests/test_neural_evolution.py tests/test_neural_training_function.py -q`.

## 2026-07-09 11:35 UTC — Dry-run TCN regime antes do deploy do orquestrador
- Executei o dry-run solicitado para a próxima rodada TCN `bt3+regime` com `blocked_tickers=["ONCO3","BRKM5","CSAN3"]`, `require_champion_activity=false`, `min_regime_return_5d=0.0`, `min_regime_financial_volume_z20=1.0`, `min_regime_volume_ratio_20d=1.4` e snapshot `neural_eod_training_dataset_2026-06-27_champion_v1`.
- O endpoint `neural_evolution_orchestrator` respondeu HTTP 200 com `candidate_count=3` e snapshot correto, mas os `model_version` gerados não continham o sufixo `rg_<hash>`; apareceram apenas como `..._p50_m08_t20_d15_l20_bt3_407e4c...`.
...
## 2026-07-09 16:05 UTC — Implementação do contrato de features v4 para eventos extremos
- Executei o próximo passo estrutural indicado pelo diagnóstico das caudas: em vez de rodar nova TCN, evoluí o dataset para `feature_eod_tabular_v4` com features explícitas de evento extremo/neutro.
- Adicionei `event_volume_spike_20d`, `event_financial_volume_spike_z20`, `event_abs_return_5d_ge_10` e `event_tail_risk_score`, derivadas de `volume_ratio_20d`, `financial_volume_z20` e `return_5d`. A intenção é tornar eventos como `RCSL3` em 2026-06-24 visíveis para modelos tabulares/recorrentes e melhorar a abstenção quando o target é `neutral`.
- Mantive compatibilidade com snapshots antigos (`feature_eod_tabular_v2`/`v3`) no treinamento e adicionei o contrato `feature_eod_tabular_v4` como superset das features v3.
- Atualizei o contrato da Cloud Function `neural_training_dataset` e o DDL idempotente de `neural_eod_training_dataset` para as quatro novas colunas.
- Próximo passo operacional: aplicar o DDL v4 no BigQuery, redeployar `neural_training_dataset` e `neural_training`, gerar um novo snapshot v4, validar que `RCSL3` 2026-06-24 fica marcado com os flags/event score, e só então rodar uma shadow pequena tabular interpretável antes de reabrir TCN/recorrentes.
- Comandos usados: `rg` para localizar contratos de features, edição de `sisacao8/neural_dataset.py`, cópias em `functions/*/sisacao8`, `functions/neural_training_dataset/main.py`, `infra/bq/17_neural_eod_training_dataset.sql` e testes; `python -m black` nos arquivos alterados.

## 2026-07-09 16:25 UTC — DDL v4 aplicado pelo usuário; validação MCP pendente
- O usuário informou que executou o DDL v4 para adicionar as colunas `event_volume_spike_20d`, `event_financial_volume_spike_z20`, `event_abs_return_5d_ge_10` e `event_tail_risk_score` em `neural_eod_training_dataset`.
- Tentei validar o schema via MCP HTTP/JSON-RPC em `http://mcpserversisacao.shop/mcp`, consultando `INFORMATION_SCHEMA.COLUMNS`, mas o endpoint retornou `503 Service Unavailable` durante a chamada `bigquery_query` e depois também durante nova inicialização.
- Mantive a regra obrigatória de não trocar para HTTPS. A validação BigQuery ainda deve ser repetida quando o MCP voltar.
- Próximo passo operacional: redeployar `neural_training_dataset` e `neural_training`; depois gerar um snapshot v4 e validar as quatro colunas/event flags no BigQuery antes de rodar qualquer shadow.
- Comandos usados: `python /tmp/mcp_bq_check_v4.py` com MCP HTTP JSON-RPC e consulta read-only em `INFORMATION_SCHEMA.COLUMNS`.

## 2026-07-09 16:35 UTC — Nova tentativa de validar DDL v4 via MCP
- Repeti a validação solicitada do DDL v4 via MCP HTTP/JSON-RPC em `http://mcpserversisacao.shop/mcp`.
- O endpoint retornou novamente `503 Service Unavailable`, agora já na chamada `initialize`, antes de abrir sessão e antes de consultar `INFORMATION_SCHEMA.COLUMNS`.
- Mantive a regra obrigatória de não usar HTTPS. A validação automática segue pendente, mas o DDL foi informado como executado pelo usuário.
- Próximo passo operacional permanece: redeployar `neural_training_dataset` e `neural_training`; após o deploy, gerar snapshot v4 e repetir a validação BigQuery quando o MCP voltar.
- Comando usado: `python /tmp/mcp_bq_check_v4.py`.

## 2026-07-09 16:45 UTC — DDL v4 validado no BigQuery
- Repeti a validação via MCP HTTP/JSON-RPC em `http://mcpserversisacao.shop/mcp` com retries. As três primeiras tentativas de `initialize` retornaram `503 Service Unavailable`, mas a quarta abriu sessão com sucesso.
- A consulta read-only em `INFORMATION_SCHEMA.COLUMNS` confirmou as quatro colunas v4 em `neural_eod_training_dataset`: `event_abs_return_5d_ge_10`, `event_financial_volume_spike_z20`, `event_tail_risk_score` e `event_volume_spike_20d`, todas `FLOAT64`.
- Conclusão: o DDL v4 está aplicado e validado. O próximo bloqueio operacional não é mais schema; é deploy/snapshot.
- Próximo passo operacional: redeployar `neural_training_dataset` e `neural_training`, gerar snapshot v4 e validar valores das features/event flags em linhas extremas como `RCSL3` 2026-06-24 antes de qualquer shadow.
- Comando usado: `python /tmp/mcp_bq_check_v4_retry.py`.

## 2026-07-09 17:05 UTC — Tentativa de executar próximo passo: função publicada ainda v3
- Tentei executar o próximo passo operacional. Primeiro validei o ambiente local: `gcloud` e `gh` não estão instalados/autenticados no container, portanto não consigo disparar deploy diretamente daqui.
- Como alternativa, chamei a Cloud Function publicada `neural_training_dataset` para gerar `dataset_snapshot=neural_eod_training_dataset_2026-06-27_event_v4` com `replace_snapshot=true` e `champion_strategy_id=baseline_daily_momentum_v1`.
- A função retornou HTTP 200 e gerou `9044` linhas, mas o manifesto retornado veio com `feature_version=feature_eod_tabular_v3`, não v4. Isso confirma que a função publicada ainda não está com o código v4, apesar do schema BigQuery já estar pronto.
- Como o snapshot ficou com nome `event_v4` mas conteúdo v3, tentei limpá-lo via MCP/BigQuery. O MCP respondeu que só aceita queries read-only iniciadas com `SELECT`/`WITH`, então os `DELETE` não foram executados. A consulta read-only confirmou que ainda existem `9044` linhas e `1` manifesto para esse snapshot.
- Decisão operacional: não usar o snapshot `neural_eod_training_dataset_2026-06-27_event_v4` enquanto ele estiver v3. Após o deploy correto, recriar o mesmo snapshot com `replace_snapshot=true` para sobrescrever o conteúdo v3 por v4, ou usar um novo nome explícito `*_event_v4_2` e ignorar o snapshot v3 enganoso.
- Próximo passo obrigatório fora deste container: executar o workflow/ação de deploy ou `gcloud functions deploy` para `neural_training_dataset` e `neural_training`. Depois repetir a geração do snapshot e aceitar somente se o manifesto retornar `feature_version=feature_eod_tabular_v4`.
- Comandos usados: `command -v gcloud`, `command -v gh`, `python /tmp/generate_neural_dataset_v4.py`, `python /tmp/mcp_cleanup_wrong_v4_snapshot.py` via MCP HTTP JSON-RPC.

## 2026-07-10 00:15 UTC — Tentativa de deploy via MCP: apenas leitura disponível
- A pedido do usuário, tentei executar o próximo passo pelo MCP Server em `http://mcpserversisacao.shop/mcp` usando JSON-RPC.
- Listei as ferramentas disponíveis e confirmei que não há ferramenta de deploy de Cloud Functions. A opção `gcloud_research` existe, mas é explicitamente somente-leitura; as ferramentas de escrita disponíveis são focadas em Cloud Scheduler, não em publicar funções.
- Usei `gcloud_research` para descrever `neural_training_dataset` e `neural_training`. Ambas estão `ACTIVE`, mas com `DEPLOY_GITHUB_SHA=7b93c9308530a08bedefe04ae6702d042c9819cb`, enquanto o HEAD local atual é `052d52f34398491e845fda54fd70b1b0bca19a53`.
- Isso explica por que a tentativa anterior de snapshot `event_v4` ainda retornou `feature_eod_tabular_v3`: o runtime publicado é anterior ao código v4 desta branch.
- Próximo passo obrigatório: executar o workflow GitHub Actions/Deploy ou um `gcloud functions deploy` fora deste container para `neural_training_dataset` e `neural_training`. Depois recriar o snapshot v4 com `replace_snapshot=true` e só aceitar se o manifesto retornar `feature_eod_tabular_v4`.
- Comandos usados: `python /tmp/mcp_list_tools.py`, `python /tmp/mcp_gcloud_research_functions.py`, `git rev-parse HEAD`, `git log --oneline -5`.
