...
- **Ingestão diária**: a função `get_stock_data` baixa o arquivo oficial diário `COTAHIST_DDDMMAAAA.ZIP` no endpoint `InstDados/SerHist/`, lê o TXT (layout fixo) e extrai **OHLCV** (open/high/low/close, volume financeiro, quantidade e número de negócios) já normalizados pelo `FATCOT`. Em seguida grava no BigQuery em `cotacao_intraday.cotacao_ohlcv_diario` com esquema explícito (`ticker`, `data_pregao`, `open`, `high`, `low`, `close`, `volume_financeiro`, `qtd_negociada`, `num_negocios`, `fonte`, `atualizado_em`). citeturn34view0  
