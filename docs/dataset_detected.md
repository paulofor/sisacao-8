...
PRICE_TABLE_DAILY=cotacao_ohlcv_diario
COL_DATETIME=candle_datetime
COL_DATE=data_pregao
COL_OPEN=open
COL_HIGH=high
COL_LOW=low
COL_CLOSE=close
...
WHERE table_name = 'cotacao_ohlcv_diario'
...
A nova tabela `sinais_eod` armazena os sinais condicionais gerados pela função `eod_signals` com schema fixo (`date_ref`, `valid_for`, `ticker`, `side`, `entry`, `target`, `stop`, `x_rule`, `y_target_pct`, `y_stop_pct`, `rank`, `model_version`, `source_snapshot`, `code_version`).
